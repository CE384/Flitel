create table country(code integer,
				 name varchar(20) not null unique,
				 primary key (code));
				 
create table city(country_code integer,
				 code integer,
				 name varchar(20) not null,
				 foreign key (country_code) references country(code),
				 primary key (country_code, code));	
				 
create table user_table (id integer,
					 type varchar(20) not null,
					 username varchar(20) not null unique,
					 password varchar(20) not null,
					 primary key (id),
					 check (type in ('customer', 'airline', 'hotel')));
					 
create table costumer (id integer,
...
create table airline (id integer,
				name varchar(20) not null unique,
				description varchar(100),
				foreign key (id) references user_table(id),
				primary key (id));			 
	
create table hotel (id integer,
				name varchar(20) not null unique,
				description varchar(100),
				phone varchar(20) check (phone ~ '^[0-9]+$'),
...
create table room(hotel_id integer,
				 number integer not null,
				 type varchar(20) check (type in ('1 bed', '2 beds', 'suite')),
				 capacity integer,
				 price integer,
				 foreign key (hotel_id) references hotel(id),
				 primary key (hotel_id, number));
				 
create table flight (airline_id integer,
					number integer not null,
					type varchar(20) check (type in ('buisiness', 'economy', 'first class')),
				    capacity integer,
				    price integer,
...
create table discount(id integer,
				 percent integer not null check (percent >= 0 and percent <= 100), 
				 primary key (id));
				 
create table booking(id integer,
					submission_date date,
					transaction_date date,
					status varchar(20) check (status in ('completed', 'cancelled', 'waiting for payment')),
					transaction_amount integer,
					costumer_id integer not null,
					foreign key (costumer_id) references costumer(id),
				 	primary key (id));
					
create table book_discount(booking_id integer,
						  discount_id integer,
						  foreign key (booking_id) references booking(id),
						  foreign key (discount_id) references discount(id),
						  primary key (booking_id, discount_id));
						  
create table flight_booking(id integer,
						  airline_id integer,
...
create table room_booking(id integer,
						  hotel_id integer,
						  room_number integer,
						  from_date date,
						  to_date date,
